...
import org.openfmb.xsd.converters.GenerationControlProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsGenerationControlProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = GenerationControlProfile.class.getName();
		...
		while (!this.shutDown)
		{
			GenerationControlProfile profile = retrieveGenerationControlProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve GenerationControlProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = GenerationControlProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = GenerationControlProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("GenerationControlProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract GenerationControlProfile retrieveGenerationControlProfile();
}
