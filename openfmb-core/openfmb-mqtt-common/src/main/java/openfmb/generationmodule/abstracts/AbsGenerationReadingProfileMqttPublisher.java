...
import org.openfmb.xsd.converters.GenerationReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsGenerationReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = GenerationReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			GenerationReadingProfile profile = retrieveGenerationReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve GenerationReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = GenerationReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = GenerationReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("GenerationReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract GenerationReadingProfile retrieveGenerationReadingProfile();
}
