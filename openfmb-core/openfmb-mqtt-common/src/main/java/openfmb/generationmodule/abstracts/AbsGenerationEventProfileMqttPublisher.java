...
import org.openfmb.xsd.converters.GenerationEventProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsGenerationEventProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = GenerationEventProfile.class.getName();
		...
		while (!this.shutDown)
		{
			GenerationEventProfile profile = retrieveGenerationEventProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve GenerationEventProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = GenerationEventProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = GenerationEventProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("GenerationEventProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract GenerationEventProfile retrieveGenerationEventProfile();
}
