 ...
 * the License.
 */
package openfmb.loadmodule.abstracts;

import javax.xml.bind.JAXBException;
import javax.xml.datatype.DatatypeConfigurationException;

import org.openfmb.xsd._2015._12.openfmb.loadmodule.LoadControlProfile;
import org.openfmb.xsd.converters.LoadControlProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsLoadControlProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = LoadControlProfile.class.getName();
		...
		while (!this.shutDown)
		{
			LoadControlProfile profile = retrieveLoadControlProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve LoadControlProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = LoadControlProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = LoadControlProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("LoadControlProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract LoadControlProfile retrieveLoadControlProfile();
}
