...
import org.openfmb.xsd.converters.LoadStatusProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsLoadStatusProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = LoadStatusProfile.class.getName();
		...
		while (!this.shutDown)
		{
			LoadStatusProfile profile = retrieveLoadStatusProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve LoadStatusProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = LoadStatusProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = LoadStatusProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("LoadStatusProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract LoadStatusProfile retrieveLoadStatusProfile();
}
