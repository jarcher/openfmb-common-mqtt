...
import org.openfmb.xsd.converters.LoadForecastProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsLoadForecastProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = LoadForecastProfile.class.getName();
		...
		while (!this.shutDown)
		{
			LoadForecastProfile profile = retrieveLoadForecastProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve LoadForecastProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = LoadForecastProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = LoadForecastProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("LoadForecastProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract LoadForecastProfile retrieveLoadForecastProfile();
}
