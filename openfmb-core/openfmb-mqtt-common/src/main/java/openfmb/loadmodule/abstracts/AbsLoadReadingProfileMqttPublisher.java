...
import org.openfmb.xsd.converters.LoadReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsLoadReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = LoadReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			LoadReadingProfile profile = retrieveLoadReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve LoadReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = LoadReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = LoadReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("LoadReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract LoadReadingProfile retrieveLoadReadingProfile();
}
