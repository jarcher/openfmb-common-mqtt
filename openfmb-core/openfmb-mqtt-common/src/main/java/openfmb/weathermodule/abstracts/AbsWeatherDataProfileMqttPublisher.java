 ...
 * the License.
 */
package openfmb.weathermodule.abstracts;

import javax.xml.bind.JAXBException;
import javax.xml.datatype.DatatypeConfigurationException;

import org.openfmb.xsd._2015._12.openfmb.weathermodule.WeatherDataProfile;
import org.openfmb.xsd.converters.WeatherDataProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsWeatherDataProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = WeatherDataProfile.class.getName();
		...
		while (!this.shutDown)
		{
			WeatherDataProfile profile = retrieveWeatherDataProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve WeatherDataProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = WeatherDataProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = WeatherDataProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("WeatherDataProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract WeatherDataProfile retrieveWeatherDataProfile();
}
