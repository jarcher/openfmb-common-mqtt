...
import org.openfmb.xsd.converters.SolarReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsSolarReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = SolarReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			SolarReadingProfile profile = retrieveSolarReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve SolarReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = SolarReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = SolarReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("SolarReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract SolarReadingProfile retrieveSolarReadingProfile();
}
