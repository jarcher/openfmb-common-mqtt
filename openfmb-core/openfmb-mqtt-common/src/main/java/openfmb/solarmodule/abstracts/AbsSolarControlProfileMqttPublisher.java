...
import org.openfmb.xsd.converters.SolarControlProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsSolarControlProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = SolarControlProfile.class.getName();
		...
		while (!this.shutDown)
		{
			SolarControlProfile profile = retrieveSolarControlProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve SolarControlProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = SolarControlProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = SolarControlProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("SolarControlProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract SolarControlProfile retrieveSolarControlProfile();
}
