...
import org.openfmb.xsd.converters.SolarForecastProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsSolarForecastProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = SolarForecastProfile.class.getName();
		...
		while (!this.shutDown)
		{
			SolarForecastProfile profile = retrieveSolarForecastProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve SolarForecastProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = SolarForecastProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = SolarForecastProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("SolarForecastProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract SolarForecastProfile retrieveSolarForecastProfile();
}
