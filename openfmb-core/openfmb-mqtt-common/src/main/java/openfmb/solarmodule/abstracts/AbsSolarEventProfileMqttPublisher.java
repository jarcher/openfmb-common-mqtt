...
import org.openfmb.xsd.converters.SolarEventProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsSolarEventProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = SolarEventProfile.class.getName();
		...
		while (!this.shutDown)
		{
			SolarEventProfile profile = retrieveSolarEventProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve SolarEventProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = SolarEventProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = SolarEventProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("SolarEventProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract SolarEventProfile retrieveSolarEventProfile();
}
