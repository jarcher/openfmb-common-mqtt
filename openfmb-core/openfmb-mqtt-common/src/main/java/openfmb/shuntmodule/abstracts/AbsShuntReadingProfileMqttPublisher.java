...
import org.openfmb.xsd.converters.ShuntReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsShuntReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = ShuntReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			ShuntReadingProfile profile = retrieveShuntReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve ShuntReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = ShuntReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = ShuntReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("ShuntReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract ShuntReadingProfile retrieveShuntReadingProfile();
}
