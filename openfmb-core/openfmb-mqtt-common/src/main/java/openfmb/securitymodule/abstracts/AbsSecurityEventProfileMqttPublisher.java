 ...
 * the License.
 */
package openfmb.securitymodule.abstracts;

import javax.xml.bind.JAXBException;
import javax.xml.datatype.DatatypeConfigurationException;

import org.openfmb.xsd._2015._12.openfmb.securitymodule.SecurityEventProfile;
import org.openfmb.xsd.converters.SecurityEventProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsSecurityEventProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = SecurityEventProfile.class.getName();
		...
		while (!this.shutDown)
		{
			SecurityEventProfile profile = retrieveSecurityEventProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve SecurityEventProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = SecurityEventProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = SecurityEventProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("SecurityEventProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract SecurityEventProfile retrieveSecurityEventProfile();
}
