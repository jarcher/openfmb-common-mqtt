 ...
 * the License.
 */
package openfmb.reclosermodule.abstracts;

import javax.xml.bind.JAXBException;
import javax.xml.datatype.DatatypeConfigurationException;

import org.openfmb.xsd._2015._12.openfmb.reclosermodule.RecloserControlProfile;
import org.openfmb.xsd.converters.RecloserControlProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsRecloserControlProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = RecloserControlProfile.class.getName();
		...
		while (!this.shutDown)
		{
			RecloserControlProfile profile = retrieveRecloserControlProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve RecloserControlProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = RecloserControlProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = RecloserControlProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("RecloserControlProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract RecloserControlProfile retrieveRecloserControlProfile();
}
