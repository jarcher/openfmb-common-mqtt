...
import org.openfmb.xsd.converters.RecloserEventProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsRecloserEventProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = RecloserEventProfile.class.getName();
		...
		while (!this.shutDown)
		{
			RecloserEventProfile profile = retrieveRecloserEventProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve RecloserEventProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = RecloserEventProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = RecloserEventProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("RecloserEventProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract RecloserEventProfile retrieveRecloserEventProfile();
}
