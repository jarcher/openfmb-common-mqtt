...
import org.openfmb.xsd.converters.RecloserReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsRecloserReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = RecloserReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			RecloserReadingProfile profile = retrieveRecloserReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve RecloserReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = RecloserReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = RecloserReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("RecloserReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract RecloserReadingProfile retrieveRecloserReadingProfile();
}
