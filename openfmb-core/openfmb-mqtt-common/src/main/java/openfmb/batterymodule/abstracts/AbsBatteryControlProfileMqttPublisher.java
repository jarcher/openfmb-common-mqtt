...
import org.openfmb.xsd.converters.BatteryControlProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsBatteryControlProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = BatteryControlProfile.class.getName();
		...
		while (!this.shutDown)
		{
			BatteryControlProfile profile = retrieveBatteryControlProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve BatteryControlProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = BatteryControlProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = BatteryControlProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("BatteryControlProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract BatteryControlProfile retrieveBatteryControlProfile();
}
