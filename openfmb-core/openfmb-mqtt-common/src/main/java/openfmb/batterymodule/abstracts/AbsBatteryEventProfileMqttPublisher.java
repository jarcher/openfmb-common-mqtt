...
import org.openfmb.xsd.converters.BatteryEventProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsBatteryEventProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = BatteryEventProfile.class.getName();
		...
		while (!this.shutDown)
		{
			BatteryEventProfile profile = retrieveBatteryEventProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve BatteryEventProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = BatteryEventProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = BatteryEventProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("BatteryEventProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract BatteryEventProfile retrieveBatteryEventProfile();
}
