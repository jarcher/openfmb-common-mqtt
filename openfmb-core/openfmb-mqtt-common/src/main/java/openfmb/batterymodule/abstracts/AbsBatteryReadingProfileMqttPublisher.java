 ...
 * the License.
 */
package openfmb.batterymodule.abstracts;

import javax.xml.bind.JAXBException;
import javax.xml.datatype.DatatypeConfigurationException;

import org.openfmb.xsd._2015._12.openfmb.batterymodule.BatteryReadingProfile;
import org.openfmb.xsd.converters.BatteryReadingProfileXsdConverter;

import openfmb.mqtt.common.AbsOpenFmbMqttPublisher;

public abstract class AbsBatteryReadingProfileMqttPublisher extends AbsOpenFmbMqttPublisher
{
	@Override
	public void run()
	{
		...
		String t = BatteryReadingProfile.class.getName();
		...
		while (!this.shutDown)
		{
			BatteryReadingProfile profile = retrieveBatteryReadingProfile();
			
			if (profile == null)
			{
				logger.info("Could not retrieve BatteryReadingProfile data. Skipping...");
			}
			else
			{
				// Publish the message
				...
				try
				{
        			...
        			{
						case JSON:
							message = BatteryReadingProfileXsdConverter.convertPojoToJson(profile, mqttIsFormattedOutput);
							break;
							
						case XML:
						default:
	            			message = BatteryReadingProfileXsdConverter.convertPojoToXml(profile, mqttIsFormattedOutput);
							break;
				...
				{
					logger.error("BatteryReadingProfileXsdConverter conversion error:", e);
				}
			}
	...
	public abstract BatteryReadingProfile retrieveBatteryReadingProfile();
}
